...
async function cargarUsuario() {
  const token = localStorage.getItem("doggie_token");
  if (!token) {
    if (btnGoLogin) btnGoLogin.style.display = "inline-flex";
  ...
  if (!res.ok) {
    localStorage.removeItem("doggie_token");
    if (btnGoLogin) btnGoLogin.style.display = "inline-flex";
    if (authUser) authUser.style.display = "none";
    return;
  }
  ...
  if (btnGoLogin) btnGoLogin.style.display = "none";
  if (authUser) authUser.style.display = "inline-flex";
  if (authName) authName.textContent = `Hola, ${data.user.name}`;
}
